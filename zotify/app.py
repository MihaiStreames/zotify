...
from zotify.const import (
    ...
    TRACK_BULK_URL,
    TRACKS,
    USER_FOLLOWED_ARTISTS_URL,
    USER_SAVED_TRACKS_URL,
)
from zotify.playlist import download_from_user_playlist, download_playlist, get_playlist_info
...
from zotify.utils import (
    ...
    regex_input_for_urls,
    split_sanitize_intrange,
    walk_directory_for_tracks,
)
...
def download_from_urls(urls: list[str]) -> int:
    ...
    pos = 7
    pbar = Printer.pbar(urls, unit="url", pos=pos, disable=not Zotify.CONFIG.get_show_url_pbar())
    pbar_stack = [pbar]
    ...
    for url in pbar:
        ...
        if all(res is None for res in result):
            Printer.hashtaged(
                PrintChannel.WARNING, f"No valid content_id found in {url}, skipping..."
            )
            continue
        ...
        if track_id is not None:
            ...
            download_playlist(
                {ID: playlist_id, NAME: get_playlist_info(playlist_id)[0]}, pbar_stack
            )
...
def search(search_term) -> None:
        ...
        if split in {"-t", "-type"}:
            ...
            for i in range(index + 1, len(splits)):
                ...
                if splits[i] not in allowed_types:
                    raise ValueError(
                        "Parameters passed after {} option must be from this list:\n{}".format(
                            split, "\n".join(allowed_types)
                        )
                    )
    ...
    if ALBUM in params["type"].split(","):
        ...
        if len(albums) > 0:
            ...
            for album in albums:
                album_data.append(
                    [counter, album[NAME], ",".join([artist[NAME] for artist in album[ARTISTS]])]
                )
                search_results.append(
                    {
                        ID: album[ID],
    ...
    pos = 7
    pbar = Printer.pbar(
        choices, unit="choice", pos=pos, disable=not Zotify.CONFIG.get_show_url_pbar()
    )
    pbar_stack = [pbar]
...
def client(args: Namespace) -> None:
    ...
    if args.file_of_urls:
        ...
        pos = 3
        pbar = Printer.pbar(
            liked_songs, unit="song", pos=pos, disable=not Zotify.CONFIG.get_show_playlist_pbar()
        )
        pbar_stack = [pbar]
        ...
        followed_artists = Zotify.invoke_url_nextable(
            USER_FOLLOWED_ARTISTS_URL, ITEMS, stripper=ARTISTS
        )
        pos = 7
        pbar = Printer.pbar(
            followed_artists, unit="artist", pos=pos, disable=not Zotify.CONFIG.get_show_url_pbar()
        )
        pbar_stack = [pbar]
        ...
        # this seems unnecessay, but the original code had this check so it gets to live another day
        elif regex_input_for_urls(args.search, non_global=True) != (
            None,
            None,
            None,
            None,
            ...
            Printer.hashtaged(
                PrintChannel.WARNING, "URL DETECTED IN SEARCH, TREATING SEARCH AS URL REQUEST"
            )
        ...
        archived_filenames = [
            PurePath(entry.strip().split("\t")[4]).stem for entry in archived_tracks
        ]
        ...
        pos = 1
        pbar = Printer.pbar(
            track_paths, unit="tracks", pos=pos, disable=not Zotify.CONFIG.get_show_url_pbar()
        )
