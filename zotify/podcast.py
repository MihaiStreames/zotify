...
def download_show(show_id, pbar_stack: list | None = None):
    ...
    pbar = Printer.pbar(
        episode_ids, unit="episode", pos=pos, disable=not Zotify.CONFIG.get_show_playlist_pbar()
    )
...
def download_episode(episode_id, pbar_stack: list | None = None) -> None:
    ...
    if podcast_name is None or episode_name is None or duration_ms is None:
        Printer.hashtaged(
            PrintChannel.ERROR,
            "SKIPPING EPISODE - FAILED TO QUERY METADATA\n" + f"Episode_ID: {str(episode_id)}",
        )
        wait_between_downloads()
        return
    ...
    with Loader(PrintChannel.PROGRESS_INFO, "Preparing download..."):
        ...
        episode_path = (
            PurePath(Zotify.CONFIG.get_root_podcast_path()) / podcast_name / f"{filename}.tmp"
        )
        create_download_directory(episode_path.parent)

        (raw, resp) = Zotify.invoke_url(
            PARTNER_URL + episode_id + '"}&extensions=' + PERSISTED_QUERY
        )
        ...
        if "anon-podcast.scdn.co" in direct_download_url or "audio_preview_url" not in resp:
            ...
            if stream is None:
                Printer.hashtaged(
                    PrintChannel.ERROR,
                    "SKIPPING EPISODE - FAILED TO GET CONTENT STREAM\n"
                    + f"Episode_ID: {str(episode_id)}",
                )
                wait_between_downloads()
                return
            ...
            for episode_file_match in Path(episode_path.parent).glob(
                episode_path.stem + ".*", case_sensitive=True
            ):
                episode_path_exists = episode_file_match.stat().st_size == total_size
                if episode_path_exists:
                    break
            if episode_path_exists and Zotify.CONFIG.get_skip_existing():
                Printer.hashtaged(
                    PrintChannel.SKIPPING,
                    f'"{podcast_name} - {episode_name}" (EPISODE ALREADY EXISTS)',
                )
                wait_between_downloads()
                return
    ...
    Printer.hashtaged(
        PrintChannel.DOWNLOADS, f'DOWNLOADED: "{filename}"\n' + f"DOWNLOAD TOOK {time_elapsed_dl}"
    )
