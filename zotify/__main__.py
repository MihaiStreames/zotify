...
def main():
    parser = argparse.ArgumentParser(
        prog="zotify", description="A music and podcast downloader needing only Python and FFMPEG."
    )
    ...
    parser.add_argument("-u", "--username", type=str, dest="username", help="Account username")
    parser.add_argument("--token", type=str, dest="token", help="Authentication token")

    parser.add_argument(
        "-ns", "--no-splash", action="store_true", help="Suppress the splash screen when loading"
    )
    parser.add_argument(
        "--debug",
        action="store_true",
    ...
    for flag in DEPRECIATED_FLAGS:
        group.add_argument(
            *flag["flags"], type=flag["type"], help=flag["help"], action="depreciated_ignore_warn"
        )

    for key in DEPRECIATED_CONFIGS:
        parser.add_argument(
            *DEPRECIATED_CONFIGS[key]["arg"],
            type=str,
