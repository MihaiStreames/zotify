...
from zotify.const import ID, ITEMS, NAME, PLAYLIST_URL, TRACK, TRACKS, TYPE, USER_PLAYLISTS_URL
from zotify.podcast import download_episode
from zotify.termoutput import PrintChannel, Printer
from zotify.track import download_track, parse_track_metadata
from zotify.utils import fill_output_template, split_sanitize_intrange, strptime_utc


def get_playlist_songs(playlist_id: str) -> tuple[list[str], list[dict]]:
    """returns list of songs in a playlist"""

    playlist_tracks = Zotify.invoke_url_nextable(
        f"{PLAYLIST_URL}/{playlist_id}/{TRACKS}", ITEMS, 100
    )
    ...
    playlist_tracks = [
        track_dict[TRACK] if track_dict[TRACK] is not None and track_dict[TRACK][ID] else None
...
def get_playlist_info(playlist_id) -> tuple[str, str]:
    ...
    (raw, resp) = Zotify.invoke_url(
        f"{PLAYLIST_URL}/{playlist_id}?fields=name,owner(display_name)&market=from_token"
    )
...
def download_playlist(playlist: dict, pbar_stack: list | None = None):
    ...
    pbar = Printer.pbar(
        playlist_tracks, unit="song", pos=pos, disable=not Zotify.CONFIG.get_show_playlist_pbar()
    )
        ...
        if m3u_dir is None:
            ...
            try:
                if len(playlist_tracks) > 0:
                    ...
                    first_track_path, _ = fill_output_template(
                        output_template, parse_track_metadata(playlist_tracks[0]), extra_keys
                    )
                    m3u_dir /= PurePath(first_track_path).parent
                if len(playlist_tracks) > 1:
                    extra_keys.update({"playlist_num": "01"})
                    second_track_path, _ = fill_output_template(
                        output_template, parse_track_metadata(playlist_tracks[1]), extra_keys
                    )
                    if PurePath(first_track_path).parent != PurePath(second_track_path).parent:
                        raise ValueError(
                            f"No shared parent directory between `{first_track_path}` and `{second_track_path}`"
                        )
            except Exception as e:
                Printer.hashtaged(
                    PrintChannel.ERROR,
                    f'FAILED TO PREDICT M3U8 DIRECTORY FOR "{playlist[NAME]}"\n'
...
def download_from_user_playlist():
    ...
    Printer.table(
        "PLAYLISTS",
        ("ID", "Name"),
        [[i + 1, playlist[NAME].strip()] for i, playlist in enumerate(users_playlists)],
    )
    ...
    pos = 5
    pbar = Printer.pbar(
        playlist_choices, unit="playlist", pos=pos, disable=not Zotify.CONFIG.get_show_url_pbar()
    )
    pbar_stack = [pbar]
