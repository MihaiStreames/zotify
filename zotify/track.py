...
from zotify.utils import (
    ...
    fetch_m3u8_songs,
    fill_output_template,
    fix_filename,
    fmt_duration,
    get_archived_song_ids,
    get_directory_song_ids,
    set_audio_tags,
    set_music_thumbnail,
...
def parse_track_metadata(track_resp: dict) -> dict[str, list[str] | str | int | bool]:
    ...
    track_metadata[ALBUM_ARTISTS] = [
        artist[NAME] for artist in track_resp[ALBUM][ARTISTS]
    ]  # list[str]
    ...
    track_metadata[IS_PLAYABLE] = (
        track_resp.get(IS_PLAYABLE, True)
    )  # bool, assume true
...
def get_track_genres(artist_ids: list[str], track_name: str) -> list[str]:
    if Zotify.CONFIG.get_save_genres():
        ...
        if len(genres) == 0:
            Printer.hashtaged(
                PrintChannel.WARNING, "NO GENRES FOUND\n" + f"Track_Name: {track_name}"
            )
            genres = [""]
        else:
            genres = list(genres)
...
def get_track_lyrics(track_id: str) -> list[str]:
    ...
    (raw, lyrics_dict) = Zotify.invoke_url(
        "https://spclient.wg.spot" + f"ify.com/color-lyrics/v2/track/{track_id}", expectFail=True
    )
    if lyrics_dict:
        try:
        ...
        if lyrics_dict["lyrics"]["syncType"] == "UNSYNCED":
            ...
            for line in formatted_lyrics:
                ...
                tss.append(
                    f"{timestamp}".zfill(5)
                    + f" {ts.split(':')[0]} {ts.split(':')[1].replace('.', ' ')}\n"
                )
...
def handle_lyrics(track_id: str, filedir: PurePath, track_metadata: dict) -> list[str] | None:
    ...
    try:
        with Loader(PrintChannel.PROGRESS_INFO, "Fetching lyrics..."):
            track_label = (
                fix_filename(track_metadata[ARTISTS][0])
                + " - "
                + fix_filename(track_metadata[NAME])
            )
        ...
        Printer.hashtaged(
            PrintChannel.SKIPPING, f'LYRICS FOR "{track_label}" (LYRICS NOT AVAILABLE)'
        )
    return lyrics
...
def update_track_metadata(track_id: str, track_path: Path, track_resp: dict) -> None:
    ...
    reliable_tags = (
        conv_artist_format(artists),
        conv_genre_format(genres),
        track_name,
        album,
        conv_artist_format(album_artists),
        release_year,
        disc_number,
        track_number,
    )
    unreliable_tags = (
        track_id,
        total_tracks if Zotify.CONFIG.get_disc_track_totals() else None,
    ...
    if not mismatches:
        Printer.hashtaged(
            ...
            f'VERIFIED:  METADATA FOR "{track_path.relative_to(Zotify.CONFIG.get_root_path())}"\n'
            + "(NO UPDATES REQUIRED)",
        )
        return
...
def download_track(
    mode: str, track_id: str, extra_keys: dict | None = None, pbar_stack: list | None = None
) -> None:
    ...
    if Zotify.CONFIG.get_download_parent_album():
        if (
            mode == "album"
            and "M3U8_bypass" in extra_keys
            and extra_keys["M3U8_bypass"] is not None
        ):
            ...
            try:
                ...
                Printer.hashtaged(
                    PrintChannel.ERROR, "FAILED TO FIND PARENT ALBUM\n" + f"Track_ID: {track_id}"
                )
    ...
    try:
        ...
        with Loader(PrintChannel.PROGRESS_INFO, "Preparing download..."):
            ...
            root_to_track, track_label = fill_output_template(
                output_template, track_metadata, extra_keys
            )
            ...
            if Zotify.CONFIG.get_temp_download_dir() != "":
                track_path_temp = PurePath(Zotify.CONFIG.get_temp_download_dir()).joinpath(
                    f"zotify_{str(uuid.uuid4())}_{track_id}.{track_path.suffix}"
                )
            ...
            if (
                track_path_exists
                and not in_dir_songids
                and not Zotify.CONFIG.get_disable_directory_archives()
            ):
                c = len(
                    [file for file in Path(filedir).iterdir() if file.match(track_path.stem + "*")]
                )
            ...
            liked_m3u8 = (
                child_request_mode == "liked" and Zotify.CONFIG.get_liked_songs_archive_m3u8()
            )
            if Zotify.CONFIG.get_export_m3u8() and track_id == child_request_id:
                m3u8_path: PurePath | None = extra_keys.get("m3u8_path")
                if liked_m3u8:
                    m3u8_path = filedir / "Liked Songs.m3u8"
                    songs_m3u = fetch_m3u8_songs(m3u8_path)
                track_m3u8_label = add_to_m3u8(
                    track_metadata[DURATION_MS], track_label, track_path, m3u8_path
                )
                if liked_m3u8:
        ...
        try:
            if not track_metadata[IS_PLAYABLE]:
                ...
                and Zotify.CONFIG.get_disable_directory_archives()
            ):
                Printer.hashtaged(
                    PrintChannel.SKIPPING,
                    f'"{PurePath(track_path).relative_to(Zotify.CONFIG.get_root_path())}" (FILE ALREADY EXISTS)',
                )
                ...
                Printer.hashtaged(
                    PrintChannel.SKIPPING, f'"{track_label}" (TRACK ALREADY DOWNLOADED ONCE)'
                )
                ...
                with (
                    open(track_path_temp, "wb") as file,
                    Printer.pbar(
                        desc=track_label,
                        total=total_size,
                        unit="B",
                    ...
                    while b < 5:
                        ...
                        if Zotify.CONFIG.get_download_real_time():
                            ...
                            delta_want = (downloaded / total_size) * (
                ...
                Printer.hashtaged(
                    PrintChannel.DOWNLOADS,
                    f'DOWNLOADED: "{PurePath(track_path).relative_to(Zotify.CONFIG.get_root_path())}"\n'
                ...
                if not in_global_songids:
                    add_to_song_archive(
                        track_metadata[ID],
                        PurePath(track_path).name,
                        track_metadata[ARTISTS][0],
                        track_name,
                    )
                if not in_dir_songids:
                    add_to_directory_song_archive(
                        track_path, track_metadata[ID], track_metadata[ARTISTS][0], track_name
                    )
            ...
            Printer.hashtaged(
                PrintChannel.ERROR,
                "SKIPPING SONG - GENERAL DOWNLOAD ERROR\n"
    ...
    try:
        ff_m = ffmpy.FFmpeg(
            global_options=[
                "-y",
                "-hide_banner",
                f"-loglevel {Zotify.CONFIG.get_ffmpeg_log_level()}",
            ],
        ...
        if isinstance(e, ffmpy.FFExecutableNotFoundError):
            ...
            reason = str(e) + "\n"
        Printer.hashtaged(
            PrintChannel.WARNING, reason + f"SKIPPING CONVERSION TO {file_codec.upper()}"
        )
