...
from zotify.const import (
    ...
    GENRE,
    LYRICS,
    M4A_CUSTOM_TAG_PREFIX,
    MP3_CUSTOM_TAG_PREFIX,
    TOTALDISCS,
    TOTALTRACKS,
    TRACKNUMBER,
    TRACKTITLE,
...
def fill_output_template(
    output_template: str, track_metadata: dict, extra_keys: dict
) -> tuple[str, str]:
    for k in extra_keys:
...
def walk_directory_for_tracks(path: str | PurePath) -> set[Path]:
    ...
    for dirpath, _dirnames, filenames in os.walk(Path(path)):
...
def regex_input_for_urls(
    search_input: str, non_global: bool = False
    ...
    base_url = (
        r"^(?:https?://)?open\.sp" + r"otify\.com(?:/intl-\w+)?/%s/([0-9a-zA-Z]{22})(?:\?si=.+?)?$"
    )
    if non_global:
...
def split_sanitize_intrange(raw_input: str) -> list[int]:
    ...
    for ids in IDranges:
        if "-" in ids:
            start, end = ids.split(
                "-"
            )  # will probably error if this is a negative number or malformed range
...
def compare_audio_tags(
    track_path: str | Path, reliable_tags: tuple, unreliable_tags: tuple
) -> list | bool:
    ...
    if len(reliable_tags) != len(reliable_tags_onfile) and not Zotify.CONFIG.debug():
        return True
    ...
    for i in range(len(reliable_tags)):
        ...
        elif str(reliable_tags[i]) != str(reliable_tags_onfile[i]):
            mismatches.append((reliable_tags[i], reliable_tags_onfile[i]))
    ...
    if sum([bool(tag) for tag in unreliable_tags]) > sum(
        [bool(tag) for tag in unreliable_tags_onfile]
    ) and not Zotify.CONFIG.get_strict_library_verify() and not Zotify.CONFIG.debug():
        return True
    ...
    for i in range(len(unreliable_tags)):
        ...
        elif str(unreliable_tags[i]) != str(unreliable_tags_onfile[i]):
            mismatches.append((unreliable_tags[i], unreliable_tags_onfile[i]))
...
def set_music_thumbnail(track_path: PurePath, image_url: str, mode: str) -> None:
    ...
    jpg_filename = (
        "cover.jpg" if "{album}" in Zotify.CONFIG.get_output(mode) else track_path.stem + ".jpg"
    )
...
def fmt_duration(
    duration: float | int,
    unit_conv: tuple[int] = (60, 60),
    connectors: tuple[str] = (":", ":"),
    smallest_unit: str = "s",
    ALWAYS_ALL_UNITS: bool = False,
) -> str:
...
def strptime_utc(dtstr) -> datetime.datetime:
    return datetime.datetime.strptime(dtstr[:-1], "%Y-%m-%dT%H:%M:%S").replace(tzinfo=datetime.UTC)
...
def wait_between_downloads() -> None:
    ...
    if waittime > 5:
        Printer.hashtaged(
            PrintChannel.DOWNLOADS, f"PAUSED: WAITING FOR {waittime} SECONDS BETWEEN DOWNLOADS"
        )
    sleep(waittime)
...
def add_to_song_archive(track_id: str, filename: str, author_name: str, track_name: str) -> None:
    ...
    if Path(archive_path).exists():
        with open(archive_path, "a", encoding="utf-8") as file:
            file.write(
                f"{track_id}\t{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\t{author_name}\t{track_name}\t{filename}\n"
            )
    else:
...
def get_directory_song_ids(download_path: str) -> list[str]:
    ...
    if Path(hidden_file_path).is_file() and not Zotify.CONFIG.get_disable_directory_archives():
        with open(hidden_file_path, encoding="utf-8") as file:
            track_ids.extend([line.strip().split("\t")[0] for line in file.readlines()])

    return track_ids


def add_to_directory_song_archive(
    track_path: PurePath, track_id: str, author_name: str, track_name: str
) -> None:
    ...
    with open(hidden_file_path, "a", encoding="utf-8") as file:
        file.write(
            f"{track_id}\t{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\t{author_name}\t{track_name}\t{track_path.name}\n"
        )


# Playlist File Utils
def add_to_m3u8(
    duration_ms: int, track_name: str, track_path: PurePath, m3u8_path: PurePath | None
) -> str | None:
    ...
    if m3u8_path is None:
        ...
        m3u8_path.name == "Liked Songs.m3u8"
    ):  # may get confused if playlist is named "Liked Songs"
        m3u8_path = track_path.parent / (Zotify.DATETIME_LAUNCH + "_zotify.m3u8")
        if (
            not Path(track_path.parent / "Liked Songs.m3u8").exists()
            or "justCreatedLikedSongsM3U8" in globals()
        ):
            m3u8_path = track_path.parent / "Liked Songs.m3u8"
            global justCreatedLikedSongsM3U8
            justCreatedLikedSongsM3U8 = True  # hacky, terrible, truly awful: too bad!
...
def fetch_m3u8_songs(m3u8_path: PurePath) -> list[str] | None:
    ...
    with open(m3u8_path, encoding="utf-8") as file:
